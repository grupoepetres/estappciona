class ParkingsController < ApplicationController
  ...
  def index
    @parkings = Parking.all
      if params[:search]
        @parkings=Parking.search(params[:search]).order("created_at DESC")
      else
      if logged_in?
        @parkings = Parking.where(id: session[:id])
       else
        @parkings = Parking.all.order('created_at DESC')
      end
    end
  end
  ...
  private
    ...
    def parking_params
      params.require(:parking).permit(:nombre, :direccion, :distrito, :precioxhora, :ubicacion)
    end
  end
